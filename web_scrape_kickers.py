...
def save_to_csv(df, year, week):
    ...
    file_path = os.path.join(directory, f"kickers_{year}_{week}.csv")
...
def scrape_years_weeks(start_year, end_year):
    for year in tqdm(range(start_year, end_year + 1), desc="Years"):
        for week in tqdm(range(1, 18), desc="Weeks"):
            url = f"https://www.footballdb.com/fantasy-football/index.html?pos=K&yr={year}&wk={week}"
...
if __name__ == "__main__":
    scrape_years_weeks(2020, 2024)  # Scrape from 2020 to 2024
